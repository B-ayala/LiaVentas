...
export function ProductIndumentaria({ productos }: ProductProps) {
  return (
    <div className="py-16">
      <h2 className="text-3xl font-bold text-center mb-12">Indumentaria</h2>
              ...
              <div className="mt-4 text-center">
                ...
                <div className="flex justify-center mb-4">
                  {producto.colores.map((color, index) => (
                    <div
                      key={`${producto.id}-color-${index}`}
                      className="w-6 h-6 rounded-full mr-2"
                      style={{ backgroundColor: color }}
                    />
                  ))}
                </div>
                <Link to={`/detalleIndumentaria`}>
